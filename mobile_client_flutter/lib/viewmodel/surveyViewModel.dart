...
class SurveyViewModel extends ChangeNotifier {
  ...
  Future<void> 
    AddSurveyWithoutAnswers(String survey_id,String title,String description, DateTime create_time, User user) async {
    try {
      setState(S_State.BUSY);
      final MutationOptions options = MutationOptions(
        document: gql(addSurveyWithoutAnswers(survey_id, title, description,create_time.toString(), user.email)),
      );
